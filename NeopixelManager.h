...
public:
    ...
    /////////////////////////////// Offsets from WeatherManager //////////////
    bool  activateHumidScaling(bool _h, bool _s, bool _b);
    bool  activateTempScaling(bool _h, bool _s, bool _b);
    bool  activateHumidOffsets(bool _h, bool _s, bool _b);
    ...
    void setLuxBS(double scaler)
    {
        lux_bs = scaler;
        lux_bs_tracker.update();
    };
    ...
    //////////////////////////////// ColorWipe /////////////////////////////////
    void colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness, double bs, int start, int end);
    void colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness, int start, int end);
...
private:
    ...
    /////////////////////// Weather //////////////////////////////////////////
    // does temperature scale hue/sat/or brightness?
    bool temp_scales[3] = {false, false, false};
    bool humid_scales[3] = {false, false, false};
    bool temp_offsets[3] = {false, false, false};
    bool humid_offsets[3] = {false, false, false};
...
bool NeoGroup::shutdown(uint16_t len)
{
    ...
    {
        dprint(p_lux, millis());
        dprint(p_lux, "\tSHUTTING DOWN GROUP ");
        dprint(p_lux, id);
...
void NeoGroup::printColors()
{
    ...
    Serial.print(rgb[2]);
    Serial.print("  =  ");
    Serial.print(rgb[0] + rgb[1] + rgb[0]);
    Serial.print("\thue ");
    Serial.print(hsb[0]);
    Serial.print("\tsat ");
    Serial.print(hsb[1]);
...
void NeoGroup::colorWipeHSB(double h, double s, double b, double brightness, double bs, int start, int end)
{
    ...
    HsbToRgb(h, s, b);
    // TODO calculate brightness properly
    colorWipe(rgb[0], rgb[1], rgb[2], brightness, bs, start, end);
}

void NeoGroup::colorWipeHSB(double h, double s, double b, double brightness, int start, int end)
{
    // dprint(p_color_wipe, "Entering colorWipeHSB() : ");
    updateHSB(h, s, b);
    HsbToRgb(h, s, b);
    // TODO calculate brightness properly
    colorWipe(rgb[0], rgb[1], rgb[2], brightness, lux_bs, start, end);
}
...
void NeoGroup::colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness, int start, int end)
{
    colorWipe(red, green, blue, brightness, lux_bs, start, end);
}

void NeoGroup::colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness)
{
    colorWipe(red, green, blue, brightness, lux_bs, 0, num_pixels);
}

void NeoGroup::colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness, double bs)
{
    colorWipe(red, green, blue, brightness, bs, 0, num_pixels);
}

void NeoGroup::colorWipe(uint16_t red, uint16_t green, uint16_t blue, double brightness, double bs, int start, int end)
{
    ...
    dprint(p_color_wipe, " - start/end: ");
    dprint(p_color_wipe, start);
    dprint(p_color_wipe, " - ");
    dprintln(p_color_wipe, end);
    dprint(p_color_wipe, " rgb (brightness/bs): ");
    ...
    else if (mapping == LED_MAPPING_INSIDE_OUT)
    {
        // center ring front: 16, 17, 18, 19
        // center ring rear: 36, 37, 38, 39
        // middle ring front: 10, 11, 12, 13, 14, 15
    ...
    {
        Serial.println("WARNING - The mapping strategy currently selected is not implemented, colorWipe is exiting now");
    }
    leds->show();
...
bool NeoGroup::flashOn(uint16_t red, uint16_t green, uint16_t blue)
{
    ...
    if (red + green + blue > 0 && shdn_timer > shdn_len)
    {
        ...
        if ((flash_on == false) || (leds_on == false))
        {
            ...
            dprint(p_onset, " Flashed ");
            dprint(p_onset, remaining_flash_delay);
            dprint(p_onset, " FPM ");
            dprintln(p_onset, fpm);
        }
        else
        { // if a flash is on then increment the remaining_flash_Delay
...
void NeoGroup::update()
{
    ...
    if (remaining_flash_delay > 0)
    {
        ...
        dprint(p_onset, "flash delay ");
        dprint(p_onset, id);
        dprint(p_onset, " : ");
        dprint(p_onset, remaining_flash_delay);
        dprintTab(p_onset);
        dprint(p_onset, last_flash_update);
        dprintTab(p_onset);
        // if the flash is not currently on, turn the flash on
        if (flash_on == false)
        { //and the light is not currently on
        ...
        else
        {
            ...
            if (remaining_flash_delay == 0)
            {
                dprint(p_onset, F("Click time over, turning off flash "));
                dprintln(p_onset, id);
...
void NeoGroup::updateOnRatio(int color)
{
    ...
    dprint(p_on_ratio, "updated led on/off ratio ");
    dprint(p_on_ratio, " :\t");
    dprint(p_on_ratio, on_ratio);
    dprint(p_on_ratio, "\t=\t");
    dprint(p_on_ratio, on_time);
    dprint(p_on_ratio, "\t");
    dprintln(p_on_ratio, off_time);
}
...
bool  NeoGroup::activateTempScaling(bool _h, bool _s, bool _b){
    temp_scales[0] = _h;
    temp_scales[1] = _s;
    temp_scales[2] = _b;
    return true;
...
bool  NeoGroup::activateHumidOffsets(bool _h, bool _s, bool _b){
    humid_scales[0] = _h;
    humid_scales[1] = _s;
    humid_scales[2] = _b;
    return true;
}
bool  NeoGroup::activateTempOffsets(bool _h, bool _s, bool _b){
    temp_scales[0] = _h;
    temp_scales[1] = _s;
    temp_scales[2] = _b;
    return true;
